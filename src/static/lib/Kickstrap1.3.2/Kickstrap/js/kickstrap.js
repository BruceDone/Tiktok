...
if (!ks.opts['console'] || typeof ks.opts['console'] != 'boolean') {
    ks.opts['console'] = false
}
...
ks['hello'] = 'KS: Hi! (' + thisVersion + ')',
    ks['edit'] = function () {
        ...
        if (typeof window.$.pnotify == 'function') {
            ...
            consoleLog('You can now edit anything in this page. (But it won\'t be saved!');
        }
    },
    ks.readyFxs = [],
    ks.ready = function (customFn) {
        this.readyFxs.push(customFn)
    },
...
function kickstrapReady(myNameIs) {
    ...
    if (appCheck) {
        ...
        for (var i = 0; i < appArray.length || function () {
            if (this.loadedLoop.every(Boolean)) ks.fire()
        }(); i++) {
...
String.prototype.isIgnored = function () {
    return (this.substr(0, 2) == "//" || this == "");
}

// For reading public items
String.prototype.isPublic = function () {
    return (this.substr(0, 5) == "http:" || this.substr(0, 6) == "https:");
}

// Differentiate JS from CSS dependencies.
String.prototype.isJS = function () {
    return (this.substr(this.length - 3, this.length) == ".js");
}
...
setTimeout(function () {
    if (!readyFired) {
        ...
        if (diagnosticMsgs.length > 0) {
            ...
            for (var i = 0; i < diagnosticMsgs.length; i++) {
                consoleLog(diagnosticMsgs[i], 'info')
            }
            consoleLog('--------------------')
        }
...
function themeFunction(urlPath) {
    $.ajax({
        type: "GET",
        url: ks.opts.rootDir + 'Kickstrap/themes/' + urlPath + '/functions.js',
        dataType: "script",
        context: self
    });
}
...
function appendMagic(newAppendee) {
    appendees = appendees.concat(newAppendee)
}
...
function initKickstrap() {
    ...
    if (!contentHack.parse) {
        for (var i = 0; i < appendees.length; i++) {
            ...
            if (scriptString == 'ndefine' || scriptString == 'on') {
                scriptString = '<script></script>'
            }
            ;
...
function app(x) {
    ...
    $.ajax({
        type: "GET", url: this.configPath, dataType: "html", context: self,
    ...
    function parseConfig(resources) {
        ...
        if (appArray.length == ks.apps.length) {
            loadResources();
        }
    }
    ...
    function loadResources() {
        if (universalsSet && ks.apps.length > 0) {
            appCheck = true;
        }
        for (var i = 0; i < appArray.length || function () {
            if (!universalsSet && appArray[0].countRequired[1] == 0) {
                finishUniversals();
            }
        }(); i++) {
            ...
            for (var j = 0; j < appArray[i].resourcesRequired.length; j++) {
                ...
                if (requiredResource.isPublic()) {
                    filePath = requiredResource;
                }
                if (!requiredResource.isIgnored()) {
                    if (requiredResource.isJS()) {
                        // Load JS
                        $.ajax({
                            type: "GET", url: (filePath), dataType: "script", context: this,
                            ...
                            success: function (data, textStatus, jqXHR) {
                                ...
                                if (currentObj.countRequired[0] == currentObj.countRequired[1]) {
                                    ...
                                    for (var k = 0; k < currentObj.resourcesDependent.length; k++) {
                                        ...
                                        if (currentObj.resourcesDependent[k].isPublic()) {
                                            filePath = currentObj.resourcesDependent[k];
                                        }
                                        if (filePath.isJS()) {
                                            $.ajax({
                                                type: "GET", url: (filePath), dataType: "script", context: this,
